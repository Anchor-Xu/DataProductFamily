...
class EnhancedDataProductOptimizer(DataProductOptimizer):
    ...
    def run_enhanced_ga(self, pop_size: int = 100, gens: int = 50, cx_rate: float = 0.85, 
        ...
        for gen in range(gens):
            ...
            # 检测收敛
            if gen >= 5:  # 从10改为5
                recent_improve = abs(np.mean(history['best_fitness'][-5:-3]) - 
                                  np.mean(history['best_fitness'][-3:]))
                if recent_improve < 1e-2 * history['best_fitness'][-1]:  # 放宽收敛阈值
    ...
    def print_config_detail(self, solution: Dict):
        ...
        dmam_names = {i: f"DMAM{i+1}（技术信息模块）" for i in range(self.DMAM)}
    ...
    def sensitivity_analysis(self, excel_path='111.xlsx', base_params=None):
        """执行灵敏度分析并将结果保存到Excel"""
        if base_params is None:
            base_params = {
                'pop_size': 50,
                'gens': 30,  # 灵敏度分析使用较少的代数以节省时间
                'cx_rate': 0.8,
                'base_mut_rate': 0.4,
                'elite_size': 5,
                'mu': 1.0
        ...
        for rate in mutation_rates:
            print(f"\n测试变异率: {rate}")
            _, fitness, history = self.run_enhanced_ga(
                pop_size=base_params['pop_size'],
                gens=base_params['gens'],
                cx_rate=base_params['cx_rate'],
                base_mut_rate=rate,
                elite_size=base_params['elite_size'],
                verbose=False
            )
            # 修复这里：直接获取集合的长度，而不是尝试求和
            used_suppliers = len(history['suppliers'][-1]) if history['suppliers'] else 0
            mut_results.append({
                '变异率': rate,
                '适应度': fitness,
                '总效用': history['total_utility'][-1],
                '总成本': history['total_cost'][-1],
                '运行时间': history['total_time'],
                '供应商使用数量': used_suppliers
            })
        
        mut_df = pd.DataFrame(mut_results)
        mut_df.to_excel(writer, sheet_name='变异率', index=False)
        
        # 2. 市场需求灵敏度分析
        print("\n进行市场需求灵敏度分析...")
        original_demand = self.market_demand.copy()
        demand_factors = [0.5, 0.75, 1.0, 1.25, 1.5]
        demand_results = []
        for factor in demand_factors:
            print(f"\n测试市场需求因子: {factor}")
            self.market_demand = original_demand * factor
            _, fitness, history = self.run_enhanced_ga(
                pop_size=base_params['pop_size'],
                gens=base_params['gens'],
                cx_rate=base_params['cx_rate'],
                base_mut_rate=base_params['base_mut_rate'],
                elite_size=base_params['elite_size'],
                verbose=False
            )
            used_suppliers = len(history['suppliers'][-1]) if history['suppliers'] else 0
        ...
        demand_df.to_excel(writer, sheet_name='市场需求', index=False)
        self.market_demand = original_demand  # 恢复原始需求
        
        # 3. 交叉概率灵敏度分析
        print("\n进行交叉概率灵敏度分析...")
        cx_rates = [0.6, 0.7, 0.8, 0.9, 1.0]
        cx_results = []
        for rate in cx_rates:
            print(f"\n测试交叉概率: {rate}")
            _, fitness, history = self.run_enhanced_ga(
                pop_size=base_params['pop_size'],
                gens=base_params['gens'],
        ...
        cx_df = pd.DataFrame(cx_results)
        cx_df.to_excel(writer, sheet_name='交叉概率', index=False)
        
        # 4. MNL模型参数灵敏度分析
        print("\n进行MNL模型参数灵敏度分析...")
        original_mu = self.mnl_param
        ...
        mu_df.to_excel(writer, sheet_name='MNL模型参数', index=False)
        self.mnl_param = original_mu  # 恢复原始参数
        
        # 保存Excel文件
        writer.close()
        print(f"\n灵敏度分析完成，结果已保存至 {excel_path}")
...
if __name__ == "__main__":
    ...
    # 新增：执行灵敏度分析
    print("\n开始灵敏度分析...")
    optimizer.sensitivity_analysis('111.xlsx')